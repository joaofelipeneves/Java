	...
	public static void main(String[] args) {
		...
		do {
			...
			case "LCO":
				...
				break;
			case "LAO":
				ListAuthorChronologicalOrder(as, in);
				break;
			case "AA":
				removeAuthorArtist(as, in);
				break;
			}
...
//OK
	private static void ListBuyOrderAndAuthorName(AlbumSystem as, Scanner in) {
		String author = in.nextLine().trim();
		IteratorLAA IteratorLAA = as.IteratorLAA(author);
...
//OK
	...
	//LAO
	public static void ListAuthorChronologicalOrder(AlbumSystem as , Scanner in) {
		String author = in.nextLine().trim();
		IteratorLAO ILAO = as.IteratorLAO(author);
		while (ILAO.hasNext()) {
			Album album = ILAO.next();
			if(album != null) {
			System.out.println(album.getAuthor() + " ; " + album.getSong() + " ; " + album.getReleaseDate());
	}
	}}

	public static void removeAuthorArtist( AlbumSystem as, Scanner in) {
		String artist = in.nextLine().trim();
		String song = in.nextLine().trim();
		as.removeAuthorArtist(artist, song);
